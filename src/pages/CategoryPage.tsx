  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-12 border-b border-white/20 pb-4">
        <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-none text-white">{category} News</h1>
        <p className="mt-4 text-white/50 text-[11px] uppercase tracking-widest font-bold">Latest updates and stories in {category}.</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map(article => (
            <Link key={article.id} to={`/article/${article.slug}`} className="group flex flex-col h-full bg-[#0a0a0a] border border-white/5 p-4 hover:border-white/20 transition-colors cursor-pointer">
              <div className="aspect-[4/3] bg-[#111] overflow-hidden relative mb-4">
                {article.coverImage ? (
                ...
                <div className="absolute top-4 left-4 bg-[#C0C0C0] text-black px-2 py-0.5 text-[9px] font-black uppercase tracking-widest shadow-sm">
                  {article.categories[0]}
                </div>
              </div>
              <div className="flex-1 flex flex-col">
                <h3 className="text-sm font-bold text-[#e5e5e5] mb-2 group-hover:text-white transition-colors leading-tight line-clamp-3">
                  {article.title}
                </h3>
                <p className="text-[11px] text-white/50 mb-4 line-clamp-2 flex-1 leading-relaxed">
                  {article.excerpt}
                </p>
                <div className="flex items-center text-[10px] text-white/40 font-medium pt-4 border-t border-white/5">
                  <span className="font-bold uppercase text-white/80">{article.authorName}</span>
